...
<template>
  <div
    ...
    :class="[positionClass]"
  >
    <div class="max-w-screen _sm:w-full pt-8 px-8 _sm:px-[5%] md:px-8 h-full flex flex-col">
      <div
        v-bind:class="type === 'sidedrawer-center' ? 'rounded-l-full bg-white py-6 mb-4' : 'mb-4'"
        v-if="!hideTitle"
        class="flex justify-between z-10 items-center"
      >
        <p class="text-black font-black text-2xl md:text-3xl lg:text-4xl">
          {{ title }}
        </p>
        <button
          type="button"
      ...
      <div class="flex flex-col overflow-hidden">
        <div v-if="!useSheet" class="px-8 _sm:px-[5%] md:px-8">
          <slot />
        </div>

        <div v-if="useSheet" class="sm:self-en z-50 bg-white pt-8 md:text-sm">
