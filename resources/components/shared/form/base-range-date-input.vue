...
const selectDay = (day: { timestamp: number }) => {
  if (selectedStartDay.value === null || (selectedEndDay.value && selectedStartDay.value)) {
...
<template>
  <div class="relative" ref="datePicker">
    <label :for="props.id" class="block text-gray-600 mb-1">{{ props.placeholder }}</label>
    <input
      type="text"
      id="date"
      readonly
    ...
    />
    <div v-if="calendarVisible" class="absolute z-50 bg-white shadow-lg rounded mt-1">
      ...
      <div class="grid grid-cols-7 gap-2">
        <div
          ...
          :class="{
            'bg-blue-200': day.timestamp === selectedStartDay?.value || day.timestamp === selectedEndDay.value,
            'bg-blue-400 text-white': (selectedStartDay.value && selectedEndDay.value) &&
              day.timestamp > selectedStartDay.value && day.timestamp < selectedEndDay.value,
