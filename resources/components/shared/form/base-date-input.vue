...
const emit = defineEmits(['update:startDate', 'update:endDate', 'validationError']);
...
const getNumberOfDays = (year: number, month: number) => new Date(year, month + 1, 0).getDate();
...
const getMonthDetails = (year: number, month: number) => {
  ...
  const monthArray = [];
  for (let i = 0; i < 42; i++) {
    const dayDetails = getDayDetails({ index: i, firstDay, year, month, numberOfDays });
    monthArray.push(dayDetails);
  }
  return monthArray;
};

const getDayDetails = ({ index, firstDay, year, month, numberOfDays }: { index: number; firstDay: number; year: number; month: number; numberOfDays: number }) => {
...
const classes = {
  ...
  inputError: "py-3 px-4 block w-full peer border-red-500 rounded-lg text-sm focus:border-red-500 focus:ring-red-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-400",
};
const inputClass = computed(() => props.error ? classes.inputError : classes.input);
...
<template>
  <div id="date_picker" class="mx-auto text-xs relative top-15" ref="datePicker">
    <div id="date_picker_input" class="mb-2">
      <label for="date" class="block text-gray-600 text-xs font-light mb-2">{{label}}</label>
      <div class="relative">
        <input
          type="text"
          id="date"
        ...
        />
        <SharedCommonIcon :name="errorMessage ? 'error':'calendar'" class="absolute top-5 right-3 size-4 pointer-events-none peer-focus:text-primary-500 text-gray-700" />
      </div>
    </div>
    <div
    ...
    >
      <div id="calendar_header" class="flex items-center justify-between mb-2 text-white">
        <button class="cal-btn bg-gray-100 dark:bg-gray-600 rounded p-1 text-gray-700 dark:text-white" @click.prevent="changeMonth(-1)">&#x3C;</button>
        <span class="text-gray-700 text-sm dark:text-white">{{ getMonthStr(month) }} {{ year }}</span>
        <button class="cal-btn bg-gray-100 dark:bg-gray-600 rounded p-1 text-gray-700 dark:text-white" @click.prevent="changeMonth(1)">&#x3E;</button>
      </div>
      ...
      <div id="calendar_main" class="grid grid-cols-7 gap-2">
        <div
          ...
          :class="[
           day.month === 0 ? 'bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white' : 'text-gray-400',
           day.timestamp === selectedDay && day.month === 0 && ' ring-2 ring-primary-300 text-primary-400 font-bold bg-primary-50'
          ]"
          @click="selectDay(day)"
        >
          <span>{{ day.date }}</span>
      ...
      <div v-if="type === 'datetime'" class="mt-4 flex items-center gap-5 border-t pt-1">
        <label class="block whitespace-nowrap text-gray-400 text-xs font-light mb-2">Select Time:</label>
        <input
          type="time"
      ...
      <div v-if="errorMessage" class="text-red-500 text-center text-[0.6rem] mt-2">{{ errorMessage }}</div>
    </div>
  </div>
</template>