...
const validateDates = () => {
  if (props.scope === 'start' && props.endDate && selectedDay.value && selectedDay.value > props.endDate) {
    errorMessage.value = 'Start date cannot be later than end date.';
    showError.value = true;
  } else if (props.scope === 'end' && props.startDate && selectedDay.value && selectedDay.value < props.startDate) {
  ...
  <div id="date_picker" class="mx-auto text-xs relative top-15" ref="datePicker">
    <div id="date_picker_input" class="mb-2">
      ...
      <div class="relative">
        ...
        />
        {{selectedDay}}{{ model }}
        <SharedCommonIcon name="calendar" class="absolute top-3 right-3 size-4 pointer-events-none text-gray-700" />
      </div>
    </div>
    <div
    ...
    >
      ...
      <div v-if="showError" class="text-red-500 text-sm mt-2">{{ errorMessage }}</div>
    </div>
  </div>
</template>
