...
const isOngoing = (props?.stage?.date?.start < (new Date()) && (new Date()) < props.stage.date.end  )
    ...
    class="grid grid-cols-7 gap-x-5 ms-1">
        ...
        >
            <div class="relative z-10 size-8 flex justify-center items-center">
                <span 
                        :class= "[isPast ? 'bg-crimson-300' : (!isOngoing && 'bg-gray-300'),isOngoing ? 'animate-ping size-4 bg-forest-200':'size-3 ' ]"

                class=" rounded-full "/>
                <span class="size-3 absolute bg-forest-300  rounded-full " v-if="isOngoing" />
