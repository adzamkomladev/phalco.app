...
const props = withDefaults(defineProps<{ accountNumber: number; wallet_id?: number }>(), {
  wallet_id: 2,
});
...
<template>
  ...
  >
    <div class="z-10 basis-1/2 justify-center flex flex-col ">
      ...
      <div class="min-h-fit">
        <p class="font-bold text-[5vw] 2xl:text-4xl md:text-[2.5vw] leading-none pt-[5%]">$5,750,30.00</p>
      </div>
    </div>
    <div class="z-10">
      <p class="text-right pb-[2%] font-medium text-[3vw] md:text-[1.5vw] 2xl:text-lg">Komla Adzam</p>
      <div class="flex justify-between">
        <p class="self-end font-mono whitespace-nowrap truncate text-[3vw] md:text-[1.5vw] 2xl:text-lg opacity-90">xxx-xxxx-xxx</p>
        <div class="text-xs flex gap-2 text-white">
          <RouterLink v-if="props.wallet_id" :href="`payments/${props.wallet_id}/withdraw`" class="py-2 px-3 font-semibold text-sm bg-[#00000050] text-nowrap rounded-md leading-none h-fit">Withdraw</RouterLink>
          <RouterLink v-if="props.wallet_id" :href="`payments/${props.wallet_id}/top-up`" class="py-2 px-3 font-semibold text-sm bg-[#00000050] text-nowrap rounded-md leading-none h-fit">Top Up</RouterLink>
        </div>
      </div>
    </div>
  </SharedCommonCard>
