...
const isFailed = computed(() => props.event.status === 'failed'|| props.event.status ==='canceled');
  ...
  <div class="p-4 flex gap-2 rounded-md hover:bg-gray-50 hover:dark:bg-gray-900 m-2 transition-all">
    ...
    <div class="grow grid gap-1">
      ...
      <SharedCommonProgressBar
        ...
        height="6px"
        :unloaded-class="[isFailed ? 'bg-crimson-100' : (isCompleted ? 'bg-teal-100' : 'bg-gray-100')]"
        :loaded-class="[isFailed  ? 'bg-crimson-500' : (isCompleted ? 'bg-teal-500' : 'bg-primary-400')]"
      />
    </div>
  </div>
</template>
