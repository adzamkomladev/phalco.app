...
const handleInput = (index: number, event: Event) => {
  ...
  if (input && index < otp.length - 1) {
    const nextInput = document.querySelectorAll<HTMLInputElement>('#otp-form input')[index + 1];
    nextInput?.focus();
  }
...
const handleKeyDown = (event: KeyboardEvent, index: number) => {
  const key = event.key;
  if (!/^[0-9]{1}$/.test(key) && key !== 'Backspace' && key !== 'Delete' && key !== 'Tab' && !event.metaKey) {
    event.preventDefault();
  }

  if ((key === 'Backspace' || key === 'Delete') && index > 0 && otp[index] === '') {
    const prevInput = document.querySelectorAll<HTMLInputElement>('#otp-form input')[index - 1];
    prevInput?.focus();
  }
};
...
const handlePaste = (event: ClipboardEvent) => {
  ...
  if (sanitizedText.length === otp.length) {
    ...
    document.querySelectorAll<HTMLInputElement>('#otp-form input').forEach((input, i) => {
      input.value = otp[i];
    });

    const lastInput = document.querySelectorAll<HTMLInputElement>('#otp-form input')[sanitizedText.length - 1];
    lastInput?.focus();
...
<template layout="auth">
  <AuthInnerLayout
    ...
    description=""
  >
  <div class="text-gray-500 flex -mt-5">Enter the 4 digit verification sent to : <div class="text-primary-500 dark:text-primary-700 pr-1">+233 345 540 643 </div> <p> to verify </p></div>
